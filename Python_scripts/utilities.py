...
def add_gene_ids(transcripts, count_arrays):
    ...
    for i, j in zip(count_arrays, range(len(gene_id))):
        i.insert(0,gene_id[j])
        i.insert(0,transcript_id[j])

def save_count_positions(count_arrays, path_to_save):
    # Calculate the longest cds region in our new list of counts
    l_tr = find_max_list(count_arrays)
...
def get_smoothed_array(array, frac = 0.05):
    '''
    a function that calculates the smoothed array of the normalized reads
    using loess and calculates the cumulative sum of said array.
    '''
    array = array + 0.000000001
    positions = np.array(list(range(len(array))))
    loess = Loess(positions, array/sum(array))
    smoothed_array = []
    for x in positions:
    ...
    cumsum = np.cumsum(smoothed_array)
    return smoothed_array, cumsum
